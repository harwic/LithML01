...
def main():
    ...
    KF = KFold(n_splits=5)
    train_indx, val_indx = [], []
    for indx1, indx2 in KF.split(tmp_X, tmp_Y):
...
def SVM_Ranker_1(X, Y, x_test, y_test, train_indx, val_indx):
    ...
    for i in ft_list:
        ...
        for j in range(0, len(train_indx)):
            ...
            y_pred = SVR(C=1).fit(x_train, y_train).predict(tmp_x_test)
            cnt = 0
            for k in range(0, len(y_pred)):
                if abs(y_pred[k] - y_test[k]) <= 0.2:
                    cnt += 1
            acc.append(cnt/len(y_pred))
...
def SVM_Ranker_2(X, Y, x_test, y_test, train_indx, val_indx):
    ...
    for i in range(0, len(train_indx)):
        ...
        y_pred = SVR(C=1).fit(x_train, y_train).predict(x_test)
        cnt = 0
        for j in range(0, len(y_pred)):
            if abs(y_pred[j] - y_test[j]) <= 0.2:
                cnt += 1
        basic_acc.append(cnt/len(y_pred))
...
def AAA(df1, ft_list):
    ...
    for i in list(df1.index):
        rk_list[ft_list.index(df1['LinR-X'][i])][1] += 1
        rk_list[ft_list.index(df1['SVR-F'][i])][1] += 1
        rk_list[ft_list.index(df1['RFR'][i])][1] += 1
        rk_list[ft_list.index(df1['XGBR'][i])][1] += 1
...
def BBB(df1, ft_list):
    ...
    for i in list(df1.index):
        rk_list[ft_list.index(df1['LinR-X'][i])][1] += 1
        rk_list[ft_list.index(df1['SVR-B'][i])][1] += 1
        rk_list[ft_list.index(df1['RFR'][i])][1] += 1
        rk_list[ft_list.index(df1['XGBR'][i])][1] += 1
        for j in range(0, len(rk_list)):
            if rk_list[j][1] == 4 and rk_list[j][2] == -1:
